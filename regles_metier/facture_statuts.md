...
|---|---|---|---|---|
| **BROUILLON → EMISE** | `emettre(factureId)` (`FactureServiceImpl.java:255`) **OU** automatique en fin de `fromReservation` / `fromCommande` / `fromBonCommande` | `statut == BROUILLON` (idempotence anti-doublon d'écriture) | `error.facture.emission.statutInvalide` (statut ≠ BROUILLON) | (1) `recordDebitOnAccountIfApplicable` : **DEBIT** compte auxiliaire client (Tour 22.1) si `TypeFacture.FACTURE` ET `clientId != null`. (2) `ecritureGenerationService.emettreEcritureFacture` génère l'écriture VTE (`411xxx` D / `706xxx` C) **uniquement pour `TypeFacture.FACTURE`** (pas AVOIR). Si échec (exercice clos…), TX rollback → reste `BROUILLON`. (3) `setEcritureEmissionId(ecritureId)` |
| **BROUILLON → ANNULEE** | `annuler(factureId)` (`FactureServiceImpl.java:294`) | `statut != ANNULEE` ET `statut != PARTIELLEMENT_PAYEE` ET `statut != PAYEE` ET `montantPaye == 0` | `error.facture.dejaAnnulee` (déjà ANNULEE) **·** `error.facture.annulation.statutInvalide` (PARTIELLEMENT_PAYEE / PAYEE) **·** `error.facture.annulation.dejaPayee` (`montantPaye > 0` cas de bord) | Pas d'écriture à contre-passer (BROUILLON n'en a jamais généré). Audit `AuditFinanceAction("FACTURE_ANNULATION")`. |
| **EMISE → ANNULEE** | `annuler(factureId)` | Identique. **Doctrine** : `ANNULEE` autorisé uniquement si **aucun paiement** n'a encore été affecté. | Mêmes clés que ci-dessus | `ecritureComptableService.contrePasser(ecritureEmissionId)` sur **exercice ouvert courant** (cf. `FactureServiceImpl.java:314-318`). |
| **EMISE → PARTIELLEMENT_PAYEE** | `PaiementServiceImpl.affecterUneAffectation(...)` (`PaiementServiceImpl.java:250`) | `statut ∈ {EMISE, PARTIELLEMENT_PAYEE}` ET `montant <= montantRestant + 0.01` ET (si `ligneFactureId` fourni) `ligne.factureId == factureId` | `error.paiement.facture.statutInvalide` (mauvais statut) **·** `error.paiement.depasseMontantRestant` (sur-affectation) **·** `error.paiement.ligne.factureMismatch` (ligne ne matche pas) | Création `AffectationPaiement`. Mise à jour `montantPaye`. **CREDIT** compte auxiliaire client (Tour 22.1) si `TypeFacture.FACTURE` + `clientId` présent. Transition décidée par : `nouveauPaye >= montantTtc` → `PAYEE`, sinon → `PARTIELLEMENT_PAYEE` (`PaiementServiceImpl.java:280-283`). |
| **EMISE → PAYEE** | Idem (paiement intégral immédiat) | `nouveauPaye >= montantTtc` | Mêmes clés que la ligne précédente | Mêmes effets que ci-dessus. |
| **EMISE → PAYEE** (cas particulier) | `ReservationFinanceServiceImpl` lors d'un check-out express avec transfert vers compte société (`ReservationFinanceServiceImpl.java:199-202`) | Le transfert est assimilé à un encaissement intégral | *(opération admin pilotée par `checkoutExpress` ; les gardes sont celles du flow check-out express, cf. `error.checkoutExpress.statut.invalid` côté réservation)* | `DEBIT` compte société + `CREDIT` compte client (si rattaché) du montant restant. `setMontantPaye(montantPaye + restant)`. |
| **PARTIELLEMENT_PAYEE → PAYEE** | `PaiementServiceImpl.affecterUneAffectation(...)` | `nouveauPaye >= montantTtc` | Mêmes clés que `EMISE → PARTIELLEMENT_PAYEE` | Idem affectation. |
...
## 9. Clés d'erreur i18n associées (récap)

Toutes les clés d'erreur retournées par les règles ci-dessus, dans un seul tableau de référence groupé par règle violée.
...
| `error.paiement.facture.statutInvalide` | Affectation paiement sur statut ≠ EMISE / PARTIELLEMENT_PAYEE |

### Opérations bloquées sur statut terminal
| Clé | Règle violée |
|---|---|
| `error.facture.statut.cloturee` | `addLigneService` sur PAYEE / ANNULEE |
| `error.facture.transfert.factureCibleTerminated` | Transfert de lignes vers facture cible PAYEE / ANNULEE |
| `error.facture.transfert.factureSourceTerminated` | Transfert de lignes depuis facture source PAYEE / ANNULEE |
| `error.facture.transfert.lignePayee` | Transfert d'une ligne ayant déjà une affectation paiement |

### Validation paiement
| Clé | Règle violée |
|---|---|
| `error.paiement.depasseMontantRestant` | Montant affecté > `montantRestant + 0.01` |
| `error.paiement.ligne.factureMismatch` | `ligneFactureId` n'appartient pas à la facture cible |

### Création / lecture
| Clé | Règle violée |
|---|---|
| `error.facture.notFound` | ID inconnu ou hors tenant |
| `error.facture.reservation.dejaFacturee` | `fromReservation` sur réservation déjà rattachée à une facture |
| `error.facture.reservation.aucuneNuiteeAFacturer` | Aucune nuitée `CONSOMMEE` non facturée |
| `error.ligneFacture.notFound` | Ligne inconnue (transfert / résolution interne) |
| `error.ligneService.requestRequired` | Payload `addLigneService` null |
| `error.ligneService.serviceInactif` | `ServiceHotelier` désactivé |
| `error.serviceHotelier.notFound` | `serviceId` inexistant ou hors tenant |
| `error.facture.transfert.requestRequired` | `transfererLignes` sans payload |
| `error.facture.transfert.lignesRequired` | `transfererLignes` sans liste de lignes |
| `error.facture.transfert.factureCibleRequired` | `transfererLignes` sans `factureCibleId` |

### Comptabilité (Bloc B1)
| Clé | Règle violée |
|---|---|
| *(remontée par `ExerciceService.assertOuvert`)* | Modification dans exercice clos (gardes Bloc B1 sur create/addLigneService) |

### Infrastructure
| Clé | Règle violée |
|---|---|
| `error.tenant.missing` | Appel d'un service `@RequireTenant` sans `TenantContext` |
| `error.user.unknown` | `currentUserId()` sans authentification valide (Spring Security) |
