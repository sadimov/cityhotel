...
/**
 * Tests Failsafe (*IT) combines pour MenageReportController (R-MEN) et
 * DirectionReportController (R-DIR) (Tour 41 P2).
 */
@SpringBootTest
@ActiveProfiles("test")
class MenageDirectionReportIT {
    ...
    void setUp() {
        ...
        userAdmin = userRepository.saveAndFlush(build("adm_dir", "adm@dir.test", mr, admin));
        userMenage = userRepository.saveAndFlush(build("men_dir", "men@dir.test", mr, menage));
        userReception = userRepository.saveAndFlush(build("rec_dir", "rec@dir.test", mr, reception));
    ...
    @Test
    @DisplayName("R-MEN-001 - MENAGE GET /recap-taches : 200")
    void shouldGetRecapTaches() throws Exception {
        String jwt = jwtFor(userMenage);
        mockMvc.perform(get("/api/reports/menage/recap-taches")
    ...
    @Test
    @DisplayName("R-MEN-002 - MENAGE GET /charge-personnel : 200")
    void shouldGetChargePersonnel() throws Exception {
        String jwt = jwtFor(userMenage);
        mockMvc.perform(get("/api/reports/menage/charge-personnel")
                        .param("from", "2026-01-01").param("to", "2026-02-01")
                        .header("Authorization", "Bearer " + jwt))
                .andExpect(status().isOk());
    }
    ...
    @Test
    @DisplayName("R-MEN - RECEPTION GET : 403")
    void shouldDenyReceptionForMenage() throws Exception {
    ...
    @Test
    @DisplayName("R-DIR-001 - ADMIN GET /direction/dashboard : 200")
    void shouldGetDashboard() throws Exception {
        String jwt = jwtFor(userAdmin);
        mockMvc.perform(get("/api/reports/direction/dashboard")
    ...
    @Test
    @DisplayName("R-DIR-001 - MENAGE GET /dashboard : 403")
